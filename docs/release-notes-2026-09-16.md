...
- The location request is sent quietly in the background and its result is ignored entirely, so a slow network, an expired session or a deploy in progress can never put an error on the punch screen for something the employee did not ask for.
